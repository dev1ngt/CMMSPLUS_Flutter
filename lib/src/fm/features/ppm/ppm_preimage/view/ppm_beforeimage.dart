...
import '../bloc/imageupload/ppm_uploadfilesbloc.dart';
import '../bloc/imageupload/ppm_uploadfilesevent.dart';
import '../bloc/imageupload/ppm_uploadfilesstate.dart';
import '../bloc/ppm_preworkbloc.dart';
import '../bloc/ppm_preworkevent.dart';
import '../bloc/ppm_preworkstate.dart';
import '../model/image_upload/ppm_uploadfile_response_model.dart';


class PPMBeforeImageView extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
    providers: [

      BlocProvider<PPMUploadFilesBloc>(
        create: (context) => PPMUploadFilesBloc(RepositoryProvider.of<ApiService>(context)),
      ),

      BlocProvider<PPMPreWorkBloc>(
        create: (context) => PPMPreWorkBloc(RepositoryProvider.of<ApiService>(context)),
      ),
...
class _CMBeforeImageState extends State<CMBeforeImageViewState> {
  ...
  final List<PPMUploadedImageData> _images = [];
  ...
  late PPMUploadFilesBloc _uploadFileBloc;
  late PPMPreWorkBloc preWorkBloc;
  ...
  void initState() {
    ...
    if( ppmSubmitRequestModel.pre_images != null){
      _images.addAll(ppmSubmitRequestModel.pre_images!);
    }
    ...
    _uploadFileBloc = BlocProvider.of<PPMUploadFilesBloc>(context);
    _uploadFileBloc.add(PPMUploadInProgressEvent());

    preWorkBloc  = BlocProvider.of<PPMPreWorkBloc>(context);
    preWorkBloc.add(PPMPreWorkLoadEvent());
    ...
    if (pickedFile != null) {
      if (pickedFile != null) {
        ...
        if (connectivityResult == ConnectivityResult.none) {
          ...
          _uploadFileBloc.add(
              PPMUploadFileInProgressEvent(tempFile, ppmid, "Before"));
        }
      }
    }
  }
  ...
  void _showImageDialog(PPMUploadedImageData imageFile) {
    showDialog(
      context: context,
      ...
      appBar: AppBar(
        ...
        actions: [
          Padding(
            ...
            child: TextButton(
              onPressed: () {
                viewModel.updatePPMWorkOrderPreImages(_images);
            ...
            children: [
              ...
              Card(
                ...
                child: Column(
                 ...
                 /* UploadImage API */
                    BlocListener<PPMUploadFilesBloc, PPMUploadFilesState>(
                      listener: (context, state) {
                        ...
                        } else if (state is PPMUploadFilesFailure) {
                          Utils.showInSnackBar(context, state.error, ToastType.Warning);
                        }
                      },
                      child: BlocBuilder<PPMUploadFilesBloc, PPMUploadFilesState>(
                        builder: (context, state) {
                          if (state is PPMUploadInProgress) {
                            return Center(
                    ...
                    BlocListener<PPMPreWorkBloc, PPMPreWorkState>(
                      listener: (context, state) {
                        if (state is PPMPreWorkSuccess) {
                        ...
                        } else if (state is PPMPreWorkFailure) {
                          Utils.showInSnackBar(context, state.error, ToastType.Warning);
                        }
                      },
                      child: BlocBuilder<PPMPreWorkBloc, PPMPreWorkState>(
                        builder: (context, state) {
                          if (state is PPMPreWorkInProgress) {
                            return Center(
                              child: CircularProgressIndicator(),
                            );
                          }
                    ...
                    TextButton(
                      onPressed: () {
                          ...
                          ToastType.Error,
                            ...
                            preWorkBloc.add(PPMPreWorkInProgressEvent(taskid));
                            print("index after>>" +
