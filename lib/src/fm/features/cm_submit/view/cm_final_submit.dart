...
import 'package:cmms/src/fm/features/cm_submit/bloc/submit/cm_submit_event.dart';
...
import '../bloc/sign/tech_sign_filesbloc.dart';
import '../bloc/sign/tech_sign_filesevent.dart';
import '../bloc/sign/tech_sign_filesstate.dart';
import '../bloc/submit/cm_submit_bloc.dart';
import '../bloc/submit/cm_submit_state.dart';

class CMFinalSubmitView extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider<TechSignFileBloc>(
          create: (context) =>
              TechSignFileBloc(RepositoryProvider.of<ApiService>(context)),
        ),
        BlocProvider<CMSubmitBloc>(
          create: (context) =>
              CMSubmitBloc(RepositoryProvider.of<ApiService>(context)),
        ),
      ],
      child: CMFinalSubmitState(),
    );
  }
}

class CMFinalSubmitState extends StatefulWidget {
  @override
  _CMFinalSubmitState createState() => _CMFinalSubmitState();
}

class _CMFinalSubmitState extends State<CMFinalSubmitState> {
  late TechSignFileBloc techSignFileBloc;
  late CMSubmitBloc cmSubmitBloc;
  String cmid = "" , userid = "";
  String? imagePath;
  ...
  void initState() {
    super.initState();
    techSignFileBloc = BlocProvider.of<TechSignFileBloc>(context);
    techSignFileBloc.add(TechSignFileInitEvent());

    cmSubmitBloc = BlocProvider.of<CMSubmitBloc>(context);
    cmSubmitBloc.add(CMSubmitInitEvent());

    try {
      CMSubmitRequestModel cmSubmitRequestModel = cmSubmitSaveModel.cmSubmitRequestModel;
      if( cmSubmitRequestModel.tecRemarks != null){
        tech_remarks_controller.text = cmSubmitRequestModel.tecRemarks!;
        if(cmSubmitRequestModel.tecSign != null && cmSubmitRequestModel.tecSign!.isNotEmpty){
          signatureSaved = true;
          imagePath    = cmSubmitRequestModel.tecSign;
        }
      }
  ...
  Widget build(BuildContext context) {
    ...
    return  WillPopScope(
      ...
      cmSubmitSaveModel.updateCMTechRemarksAndSign( tech_remarks_controller.text,
        imagePath!,);
      return true;
    },
    child: Scaffold(
              ...
              onPressed: () {
                ...
                if(tech_remarks_controller.text == ""){
                  Utils.showInSnackBar(
                      context, "Enter the remarks", ToastType.Warning);
                }
                  ...
                  if (connectivityResult == ConnectivityResult.none) {
                    ...
                    cmSubmitSaveModel.updateCMTechRemarks(
                        tech_remarks_controller.text,
                        imagePath!,
                        work_end_date_time, int.parse(userid));
                    cmSubmitBloc.add(CMSubmitInProgressEvent(
                        cmSubmitSaveModel.cmSubmitRequestModel));
                  }
                }
              },
            ...
            children: [
              Card(
                ...
                child: Column(
                  ...
                  children: [
                    BlocListener<TechSignFileBloc, TechSignFileState>(
                      listener: (context, state) {
                        if (state is TechSignFileSuccess) {
                          Utils.showInSnackBar(
                              context,
                        ...
                        } else if (state is TechSignFileFailure) {
                          Utils.showInSnackBar(
                              context, state.error, ToastType.Warning);
                        }
                      },
                      child: BlocBuilder<TechSignFileBloc, TechSignFileState>(
                        builder: (context, state) {
                          if (state is TechSignFileInProgress) {
                            return Center(
              ...
              BlocListener<CMSubmitBloc, CMSubmitState>(
                listener: (context, state) {
                  if (state is CMSubmitSuccess) {
                    Utils.showInSnackBar(
                        context, state.submitResponseModel.message, ToastType.Success);
                    cmSubmitSaveModel.resetCMData();
                    Navigator.pushNamed(context, '/dashboard');
                  } else if (state is CMSubmitFailure) {
                    Utils.showInSnackBar(
                        context, state.error, ToastType.Warning);

                  }
                },
                child: BlocBuilder<CMSubmitBloc, CMSubmitState>(
                  builder: (context, state) {
                    if (state is CMSubmitInProgress) {
                      return Center(
                        child: CircularProgressIndicator(),
                      );
                    }
    ...
    if (signatureImageBytes != null) {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        techSignFileBloc.add(
            TechSignFileInProgressEvent(sign, cmid, "Occupant"));
      }
