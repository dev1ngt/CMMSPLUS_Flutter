...
import '../bloc/ppm_details_event.dart';
import '../bloc/ppm_details_state.dart';
import '../model/ppm_details_response_model.dart';
import '../model/ppm_details_submit_request.dart';
import '../model/ppm_details_temp_data.dart';
import 'package:path/path.dart' as path;
import 'package:http/http.dart' as http;


class PPMDetails extends StatelessWidget {
  const PPMDetails({super.key});
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      ...
      child: PPMDetailsState(),
    );
  }
}

class PPMDetailsState extends StatefulWidget {
  const PPMDetailsState({super.key});

  @override
  State<PPMDetailsState> createState() => _PPMDetailsStateState();
}
                    ...
                    BlocListener<MultipleAssetsBloc, MultipleAssetsState>(
                      listener: (context, state) async {
                        if (state is MultipleAssetsLoadedState) {
                          ...
                          //     state.asset_response.message, ToastType.Success);
                          assetid_List.clear();
                          assetname_list.clear();
                    ...
                    BlocListener<DetailsPart2Bloc, DetailsPart2State>(
                      listener: (context, state) async {
                        if (state is SignUploadSuccess) {
                          ...
                          setState(() {
                            if(signType == "Tech"){
                              _techSignPath = state.fileuploadresponse.uploadedpath.toString();
                            }
                            else if(signType == "Client"){
                              _cilentSignPath = state.fileuploadresponse.uploadedpath.toString();
                            }
                      ...
                      child: ElevatedButton(
                        onPressed: isSubmitting
                          ...
                          if (description.isEmpty) {
                            ...
                            ppmsubmit.subScheduleId = int.parse(ppmid);
