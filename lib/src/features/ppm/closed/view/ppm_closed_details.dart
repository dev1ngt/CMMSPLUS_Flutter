...
import '../../completed/bloc/ppm_det_completed_bloc.dart';
import '../../completed/bloc/ppm_det_completed_event.dart';
import '../../completed/bloc/ppm_det_completed_state.dart';
...
class PPMClosedDetails extends StatelessWidget {
  const PPMClosedDetails({super.key});
    ...
    return MultiBlocProvider( // Use MultiBlocProvider to provide multiple BLoCs
      providers: [
        ...
        BlocProvider<PPMCompletedBloc>(
          create: (context) => PPMCompletedBloc(RepositoryProvider.of<ApiService>(context)),
        ),
      ],
      child: PPMClosedDetailsState(),
    );
  }
}


class PPMClosedDetailsState extends StatefulWidget {
  const PPMClosedDetailsState({super.key});

  @override
  State<PPMClosedDetailsState> createState() => _PPMClosedDetailsState();
}
...
class _PPMClosedDetailsState extends State<PPMClosedDetailsState> {
  ...
  String _selectedSignPath = "",
      QRResult = "" , webview_path = "";
  ...
  String _techSignPath  = "" , _cilentSignPath = "", vendorName = "" , signType = "";
  ...
  bool isDataAlreadyPresent(String data, List<String> assetnameList, List<int> assetIDList) {
  ...
  void initState() {
    ...
    // setState(() {
    //   PPMDetailsTempModel ppmDetailsTempModel = ppmDetailsTempSingletonModel.ppmDetailsTempModel;
  ...
  Future<void> fetchUserID() async {
    ...
    ppmid = int.parse(await AppSharedPrefs.getSubSchduleID());
  ...
  _imageFromCamera() async {
    ...
    if (pickedImage != null && imageConstraint(File(pickedImage.path))) {
      ...
      _uploadFileBloc.add(UploadFileInProgressEvent(selectedImageFile, "Test.png"));
...
// Access parameters here
    ...
    //   final String vendor = args['Vendor'] as String;
    //   vendorName = vendor;
    //   textVendorController.text = vendorName;
    // }
    ...
    if(args != null && args.containsKey("PPMID")){
      ...
      assetInput.id = ppmid_;
      //ppmid = int.parse(ppmid_);
      if (!_multiassetsApiCalled) {
        _multiassetsApiCalled = true;
        multipleAssetsBloc.add(MultipleAssetsFetchEvent(assetInput));
        ppmCompletedBloc.add(PPMCompletedStartEvent(sub_schedule_id: ppmid_));
        ppmDetailsBloc.add(PPMDetailsLoadEvent(ppmid_));
      }
                ...
                children: [
                  GestureDetector(
                    onTap: () async {
                      ...
                      final ppmlist =  await  Navigator.pushNamed(context, '/ppmList');
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      listener: (context, state) async {
                        if (state is AssetScanLoadedState) {
                          ...
                          if(isDataAlreadyPresent(name ,assetname_list , assetid_List)){
                            ...
                            Utils.showInSnackBar(context, "Scanned asset already available in list", ToastType.Warning);
                          }
                          else {
                            print("data not avaialble");
                            setState(() {
                              assetname_list.add(state.asset_response.assetData.assetName);
                              assetid_List.add(state.asset_response.assetData.assetId);
                              ppmDetailsTempSingletonModel.updateAssetData(assetid_List, assetname_list);
                            });
                          }
                    ...
                    BlocListener<MultipleAssetsBloc, MultipleAssetsState>(
                        ...
                        if (state is MultipleAssetsLoadedState) {
                          ...
                          if(state.asset_response.assets.length > 0){
                            ...
                            assets.forEach((asset) {
                              print('Asset ID: ${asset.assetId}, Asset Name: ${asset.assetName}');
                      ...
                      child: BlocBuilder<MultipleAssetsBloc, MultipleAssetsState>(
                          builder: (context, state) {
                        ...
                        if (state is PPMCompletedSuccess) {
                          setState(() {
                            ...
                            if(state.schedule.data.finalDocument.isEmpty){
                              ...
                              visibilityAdditionalDocument = !visibilityAdditionalDocument;
                            }
                            else {
                              AdditionalDocUploadPath = state.schedule.data.finalDocument;
                              visibilityAdditionalDocument = !visibilityAdditionalDocument;
                            }

                            if(state.schedule.data.technicianDescription.isNotEmpty){
                              _descriptionController.text = state.schedule.data.technicianDescription;
                            }

                            if(state.schedule.data.technicianSign.isNotEmpty){
                              _techSignPath = state.schedule.data.technicianSign;
                            }

                            if(state.schedule.data.clientSignature.isNotEmpty){
                              _cilentSignPath = state.schedule.data.clientSignature;
                            }

                            if(state.schedule.data.trackData.length > 0) {
                              for (var trackItem in state.schedule.data.trackData) {
                                photoSets.add({
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            if(webview_path != null && webview_path.isNotEmpty)  {
                              setState(() {
                              ...
                              Utils.showInSnackBar(context,
                                  "Invalid WebView", ToastType.Warning);
                            }
                      ...
                      child: Container(
                        ...
                        InAppWebView(
                          initialUrlRequest: URLRequest(
                              url: WebUri(webview_path)
                          ),
                          initialOptions: InAppWebViewGroupOptions(
                            crossPlatform: InAppWebViewOptions(
                              // Enable camera usage in the webview
                          ...
                          onPermissionRequest: (controller, request) async {
                            // if (request.permissionType == PermissionType.MEDIASTREAM_CAMERA) {
                            //   return PermissionRequestResponse(
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  image: DecorationImage(
                                    image: AdditionalDocUploadPath != null && AdditionalDocUploadPath!.isNotEmpty
                                        ? NetworkImage(AdditionalDocUploadPath!) as ImageProvider<Object>
                                        : AssetImage('assets/images/photo_man.png'),
                                    fit: BoxFit.cover,
                                  ),
                                ),
                              ),
                    ...
                    BlocListener<UploadFilesBloc, UploadFilesState>(
                      listener: (context, state) async {
                        if (state is UploadFilesSuccess) {
                          Utils.showInSnackBar(
                              context, state.fileuploadresponse.message!, ToastType.Success);
                          try{
                            setState(() {
                              if(MultiplePhotosCurrentClick == 'before'){
                                photoSets[currentIndex]['beforeImage'] = state.fileuploadresponse.uploadedpath!;
                              }
                              else if(MultiplePhotosCurrentClick == "after"){
                                photoSets[currentIndex]['afterImage'] = state.fileuploadresponse.uploadedpath!;
                              }
                              else {
                                AdditionalDocUploadPath = state.fileuploadresponse.uploadedpath!;
                                visibilityAdditionalDocument = !visibilityAdditionalDocument;
                              }
                            });
                          }catch(e){
                                        ...
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            GestureDetector(
                                              onTap: () {
                                                currentIndex = index;
                                                MultiplePhotosCurrentClick = "before";
                                                _showPhotoDialog(context);
                                              },
                                              child: Container(
                                                width: 150.0,
                                                ...
                                                decoration: BoxDecoration(
                                                  image: DecorationImage(
                                                    image: photoSets[index]['beforeImage'] != null &&
                                                        photoSets[index]['beforeImage']!.isNotEmpty
                                                        ? NetworkImage(photoSets[index]['beforeImage']!) as ImageProvider<Object>
                                                        : AssetImage('assets/images/photo_man.png'),
                                                    fit: BoxFit.cover,
                                                  ),
                                                ),
                                      ...
                                      Expanded(
                                        child: Column(
                                          ...
                                          children: [
                                            ...
                                            GestureDetector(
                                              onTap: () {
                                                MultiplePhotosCurrentClick = "after";
                                                currentIndex = index;
                                                _showPhotoDialog(context);
                                              },
                                              child: Container(
                                                ...
                                                decoration: BoxDecoration(
                                                  image: DecorationImage(
                                                    image: photoSets[index]['afterImage'] != null &&
                                                        photoSets[index]['afterImage']!.isNotEmpty
                                                        ? NetworkImage(photoSets[index]['afterImage']!) as ImageProvider<Object>
                                                        : AssetImage('assets/images/photo_man.png'),
                                                    fit: BoxFit.cover,
                                                  ),
                                                ),
                      ...
                      listener: (context, state) async {
                        if (state is PPMDetailsSubmitUploadSuccess) {
                          ...
                          if(state.submitResponseModel.status!){
                            Utils.showInSnackBar(context, state.submitResponseModel.message!, ToastType.Success);
                            Navigator.pushNamed(context, '/dashboard', );
                          }
                          else {
                            Utils.showInSnackBar(context, state.submitResponseModel.message!, ToastType.Warning);
                          }
                      ...
                      child: ElevatedButton(
                        onPressed: isSubmitting
                          ...
                          if(description.isEmpty){
                            Utils.showInSnackBar(context, "Enter the description", ToastType.Error);
                          }
                          else {
                            ...
                            PPMSubmitRequestModel ppmsubmit = PPMSubmitRequestModel();
                            ppmsubmit.finalDocument = AdditionalDocUploadPath;
                            ppmsubmit.technicianDescription = description;
                            ppmsubmit.subScheduleId = ppmid;
                            ppmsubmit.userId =  int.parse(userid);
                            ppmsubmit.trackData = convertMapListToModelList(photoSets);
                            ...
                            ppmDetailsBloc.add(PPMDetailsSubmitEvent(ppmsubmit));
                        ...
                        style: ElevatedButton.styleFrom(
                          ...
                          padding: EdgeInsets.zero, // Remove padding to allow the Container to take the entire button space
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(10.0),
                          ),
                        ),
                        child: Ink(
                          decoration: BoxDecoration(
                            gradient: LinearGradient(
                              colors: [customColor2, customColor1], // Replace with your gradient colors
                            ),
                            borderRadius: BorderRadius.circular(10.0),
                          ),
                          child: Container(
...
*/
  ...
  List<TrackDatum> convertMapListToModelList(List<Map<String, String>> mapList) {
