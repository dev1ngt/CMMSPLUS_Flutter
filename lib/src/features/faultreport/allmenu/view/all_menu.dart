...
class FRAllMenu extends StatelessWidget {
  const FRAllMenu({super.key});
...
class FRAllMenuStf extends StatefulWidget {
  const FRAllMenuStf({super.key});

  @override
  State<FRAllMenuStf> createState() => _FRAllMenuStf();
}

class _FRAllMenuStf extends State<FRAllMenuStf> {
                        ...
                        BlocBuilder<FRFirstMenuBloc, FRFirstMenuState>(
                          builder: (context, state) {
                            if (state is FRFirstMenuInitial) {
                              ...
                              List<AllType> filteredItems = state
                                  .ppmlist.allTypes
                                  .where((item) => item.requestType
                                  .toLowerCase()
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return InkWell(
                                      onTap: () {
                                        try {
                                          ...
                                          if (Is_list1.contains("1")) {
                                            viewModel.updateFaultReportSaveModel11(
                                              item.id,
                                              item.requestType,
                                            );
                                          } else {
                                            viewModel.updateFaultReportSaveModel1(
                                              item.id,
                                              item.requestType,
                                            );
                                          }
                                      ...
                                      child: Card(
                                        color: isSelected ? Color(0xFFCBD4F4) : Colors.white, // highlight selected
